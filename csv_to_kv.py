...
if len(argv) < 3:
    print  argv
    print "Usage: %s dbname filename" %argv[0]
    exit(0)
else:
    dbname=argv[1]
    csvfile=argv[2]
...
try:
  ...
  while 1 :
    ...
    # read key value from the dict
       dpfh.write(str(k).encode('utf-8')); #中文写入
       dpfh.write(': ')
       dpfh.write(str(v).encode('utf-8'))
       dpfh.write('\n')
